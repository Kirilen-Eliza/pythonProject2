...
def test_filter_by_default_state(transactions):
...
def test_filter_by_non_default_state(transactions):
...
def test_filter_by_state_wrong_value(value, expected):
    """Функция-тест, проверяет пустые входные данные"""
    assert filter_by_state(value) == expected


def test_sort_by_default_date(transactions):
...
def test_sort_by_non_default_date_(transactions):
...
def test_sort_by_date_type_wrong_value():
