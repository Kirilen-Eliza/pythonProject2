...
def test_filter_by_state(transactions):
    ...
    по умолчанию 'state'."""
    assert filter_by_state(transactions) == [{'id': 41428829, 'state': 'EXECUTED',
...
def test_filter_by_state_2(transactions):
    ...
    при вызове функции."""
    assert filter_by_state(transactions, 'CANCELED') == [{'id': 594226727, 'state': 'CANCELED',
...
def test_sort_by_date_false(transactions):
    ...
    assert sort_by_date(transactions, False) == [{'id': 939719570, 'state': 'EXECUTED',
                                                  'date': '2018-06-30T02:08:58.425572'},
