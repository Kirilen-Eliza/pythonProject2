...
def test_log_output_to_the_console(capsys):
    ...
    assert result_not_positive.out == "my_function error: division by zero. Inputs: ((10, 0), {})\n"
...
def test_log_file():
    ...
    with open("mylog.txt", "r", encoding="utf-8") as file:
        ...
        assert result_unsupported_type == (
            "my_function error: unsupported operand" " type(s) for /: 'str' and 'int'. Inputs: (('10', 5), {})"
        )
    os.remove("mylog.txt")
